...
<8> Der Test der Registrierungslogik basiert dann wieder auf `@TestFactory`, um kompakt alle Testfälle abdecken zu können. Ich arbeite an dieser Stelle mit Template-Methoden, die mir das zu testende Objekt, zum Beispiel ein Event in der Vergangenheit (`pastEvent`), erzeugen. Diese Methoden sind hier nicht weiter dargestellt.
